...
export const aiOptimizePlugin: NodePlugin = {
  ...
  executor: {
    ...
    inputSchema: {
      ...
      rawKeywords: { type: 'string', label: '完整关键词数据 JSON（含搜索量、难度等）' },
    },
    ...
    async execute(ctx) {
      ...
      const configDescription = ctx.config.productDescription as string | undefined;
      const configBulletPoints = ctx.config.productBulletPoints as string | undefined;
      const configLongDescription = ctx.config.productLongDescription as string | undefined;
      ...
      const inputLongDescription = ctx.input.longDescription as string | undefined;
      const images = (ctx.input.images as string[]) || [];
      const inputKeywords = (ctx.input.keywords as string[]) || [];
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const inputRawKeywords = (ctx.input.rawKeywords as any[]) || [];

      // 使用配置中的数据，如果没有配置则使用上游数据
      const title = configTitle || inputTitle;
      const description = configDescription || inputDescription;
      const bulletPointsConfig = configBulletPoints?.split('\n').map(s => s.trim()).filter(s => s) || inputBulletPoints;
      const longDescription = configLongDescription || inputLongDescription;

      ctx.logger('info', `使用数据源: title=${configTitle ? '配置' : '上游'}, description=${configDescription ? '配置' : '上游'}`);
      if (inputRawKeywords.length > 0) {
        ctx.logger('info', `注入 ${inputRawKeywords.length} 个竞品关键词（含搜索量、难度等数据）`);
      ...
      if (!effectiveDescription && bulletPointsConfig && bulletPointsConfig.length > 0) {
        effectiveDescription = bulletPointsConfig.join('\n');
        ctx.logger('info', `使用 bulletPoints (${bulletPointsConfig.length} 项)`);
      }

      if (!title && !effectiveDescription) {
        ctx.logger('error', `输入验证失败: 请在节点配置中输入商品信息，或连接上游节点`);
        throw new Error('请提供商品标题或描述作为输入（可以在节点配置中直接输入，或连接上游节点）');
      }

      const useImage = ctx.config.useImage as boolean && images.length > 0;
      const headless = ctx.config.headless as boolean ?? true;
      const responseTimeout = ((ctx.config.responseTimeout as number) || 60) * 1000;
      ...
      if (inputRawKeywords.length > 0) {
        ...
        keywordBlock = `\n## 竞品搜索关键词\n${inputKeywords.slice(0, 30).map((k, i) => `${i + 1}. ${k}`).join('\n')}`;
      }
      ...
      if (ctx.input.asin && ctx.input.bulletPoints && !competitorData.some(c => c.title === ctx.input.title)) {
      ...
      if (topCompetitors.length > 0) {
        const competitorSections = topCompetitors.map((comp, i) => {
          ...
          const bulletText = bullets.length > 0
            ? bullets.map((b, j) => `  ${j + 1}. ${String(b).slice(0, 150)}`).join('\n')
            : '  (无)';
...
${bulletText}${longDesc ? `\n- 长描述摘要: ${longDesc}` : ''}`;
      ...
      if (topCompetitors.length > 0) {
        ...
        ctx.logger('info', `未检测到竞品 Listing 数据（需要 amazon-product 上游节点）`);
      }
...
- 建议的定价策略参考（基于竞品关键词定位）`;
      ...
      async function callGeminiAPI(promptText: string, signal: AbortSignal): Promise<{ response: string }> {
        if (useImage && images.length > 0 && images[0] && !images[0].startsWith('http')) {
          ctx.logger('info', `[Gemini] 使用图片优化: ${images[0]}`);
          const resp = await fetch('/api/gemini/upload', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ filePath: images[0], prompt: promptText, headless, responseTimeout }),
            signal,
          });
          if (!resp.ok) { let errMsg = `HTTP ${resp.status}`; try { const err = await resp.json(); errMsg = err.error || errMsg; } catch {} throw new Error(errMsg); }
        ...
        const resp = await fetch('/api/gemini/upload', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ filePath: placeholderImagePath, prompt: promptText, headless, responseTimeout }),
          signal,
        });
        if (!resp.ok) { let errMsg = `HTTP ${resp.status}`; try { const err = await resp.json(); errMsg = err.error || errMsg; } catch {} throw new Error(errMsg); }
        return { response: (await resp.json()).response };
      ...
      async function callChatGPTAPI(promptText: string, signal: AbortSignal): Promise<{ response: string }> {
      ...
      function parseResponse(responseText: string): {
        optimizedTitle: string; optimizedDescription: string; seoKeywords: string[];
        optimizedBulletPoints: string[]; optimizedLongDescription: string; competitorAnalysis: string;
      } {
        ...
        for (const line of lines) {
          if (line.startsWith('###') || line.startsWith('##') || line.startsWith('#')) {
            ...
            else if (line.includes('长描述') || line.includes('描述')) currentSection = 'description';
            else if (line.includes('搜索关键词') || line.includes('Search Terms')) currentSection = 'keywords';
          ...
          if (line.match(/^\d+[\.\)]\s/) && currentSection === 'bullets') {
            ...
            optimizedTitle = line.split('：').slice(1).join('：').trim() || optimizedTitle;
          } else if (line.includes('关键词') || line.toLowerCase().includes('keywords')) {
            if (currentSection === 'keywords') {
              const keywords = line.split(/[，、,]/).map(k => k.trim()).filter(k => k.length > 1 && !k.includes('：'));
        ...
        return {
          ...
          seoKeywords: seoKeywords.length > 0 ? seoKeywords : ['premium', 'quality'],
      ...
      if (executionMode === 'parallel') {
        ...
        if (!geminiOk && !chatgptOk) {
          throw new Error(`Gemini: ${geminiResult.reason?.message || 'failed'}; ChatGPT: ${chatgptResult.reason?.message || 'failed'}`);
        }

        const emptyParsed = { optimizedTitle: '', optimizedDescription: '', seoKeywords: [] as string[], optimizedBulletPoints: [] as string[], optimizedLongDescription: '', competitorAnalysis: '' };
        const geminiParsed = geminiOk ? parseResponse(geminiResponse) : emptyParsed;
        const chatgptParsed = chatgptOk ? parseResponse(chatgptResponse) : emptyParsed;

        if (!geminiOk) ctx.logger('error', `Gemini 失败: ${geminiResult.reason?.message}`);
        if (!chatgptOk) ctx.logger('error', `ChatGPT 失败: ${chatgptResult.reason?.message}`);

        ctx.logger('success', `并行优化完成 (Gemini: ${geminiOk ? '成功' : '失败'}, ChatGPT: ${chatgptOk ? '成功' : '失败'})`);
      ...
      // 默认：仅 Gemini
      ctx.logger('info', useImage && images.length > 0 ? `使用图片辅助优化文案...` : `使用 Gemini 优化文案...`);
      ...
      ctx.logger('info', `新标题: ${parsed.optimizedTitle.substring(0, 50)}...`);
      return {
        optimizedTitle: parsed.optimizedTitle,
        optimizedDescription: parsed.optimizedDescription,
        optimizedBulletPoints: parsed.optimizedBulletPoints,
...
export const aiVisionPlugin: NodePlugin = {
  ...
  executor: {
    ...
    configSchema: {
      templateId: {
        type: 'select', label: '识别模板', required: true,
        options: [
      ...
      customPrompt: {
        type: 'string', label: '自定义提示词（留空则使用模板）',
      },
      maxImages: {
        type: 'number', label: '最大处理图片数', default: 1,
      },
    },
    ...
    async execute(ctx) {
      ...
      const templateId = (ctx.config.customPrompt as string)
        ? undefined
        : (ctx.config.templateId as string) || 'extract-features';
      const customPrompt = ctx.config.customPrompt as string | undefined;
      const maxImages = (ctx.config.maxImages as number) || 5;
      ...
      for (let i = 0; i < toProcess.length; i++) {
        ...
        ctx.logger('info', `识别图片 ${i + 1}/${toProcess.length}...`);
        ...
        if (!resp.ok) {
          let errMsg = `图片 ${i + 1} 识别失败: HTTP ${resp.status}`;
        ...
        results.push(data.result as string);
        ctx.logger('success', `图片 ${i + 1} 识别完成`);
      }

      ctx.logger('success', `全部完成，成功 ${results.filter(r => !r.startsWith('[错误]')).length}/${toProcess.length}`);
...
export const amazonSearchPlugin: NodePlugin = {
  ...
  executor: {
    ...
    configSchema: {
      keyword: {
        type: 'string', label: '搜索关键词（留空则从 AI 识别结果中提取）',
      },
      maxResults: {
        type: 'number', label: '最大结果数', default: 10,
      },
      useFirstLine: {
    ...
    async execute(ctx) {
      // 优先用 config 中的关键词，否则从 AI 识别结果中提取
      ...
      if (!keyword) {
        ...
        if (aiResult) {
          if (ctx.config.useFirstLine) {
            keyword = aiResult.split('\n').map(l => l.replace(/^[\d\.\-\*\s]+/, '').trim()).find(l => l.length > 3);
          }
          if (!keyword) keyword = aiResult.split('\n')[0]?.replace(/^[\d\.\-\*\s]+/, '').trim();
        }
      }

      if (!keyword) {
        throw new Error('未提供搜索关键词：请在节点配置中填写，或确保上游 AI 识别节点输出了关键词');
      }
...
export const amazonProductPlugin: NodePlugin = {
  id: 'amazon-product',
  label: 'Amazon 商品详情',
  description: '抓取 Amazon 商品完整详情（标题、价格、品牌、规格参数、五点描述等）',
  icon: 'data',
  category: 'browser',
  nodeType: 'step',
  panelGroup: 'browser',
  ...
  executor: {
    ...
    inputSchema: {
      asins: { type: 'string[]', label: '上游 ASIN 列表（来自 Amazon 竞品搜索）' },
      links: { type: 'string[]', label: '上游链接列表（来自 Amazon 竞品搜索）' },
    },
    ...
    configSchema: {
      asin: {
        type: 'string', label: 'ASIN（手动输入，优先级高于上游数据）',
      },
      productUrl: {
        type: 'string', label: '商品链接（手动输入）',
      },
      headless: {
        type: 'boolean', label: '无头模式', default: true,
      },
      saveToDb: {
        type: 'boolean', label: '保存到数据库', default: true,
      },
    },
    ...
    async execute(ctx) {
      ...
      if (!asin && !url) {
        throw new Error('未提供 ASIN 或商品链接：请在节点配置中填写，或确保上游搜索节点已执行');
      }
      ...
      return {
        ...
        specifications: Object.entries(product.specifications || {}).map(([k, v]) => `${k}: ${v}`),
...
export const xiyouzhaociPlugin: NodePlugin = {
  ...
  label: '西柚找词 - 关键词挖掘',
  description: '从西柚找词抓取 Amazon 商品关键词数据（搜索量、竞争度、流量份额等）',
  icon: 'search',
  category: 'browser',
  nodeType: 'step',
  panelGroup: 'browser',
  ...
  executor: {
    ...
    async execute(ctx) {
      ...
      if (!asin) {
        throw new Error('未提供 ASIN：请在节点配置中填写，或连接上游 Amazon 商品详情节点');
      }
