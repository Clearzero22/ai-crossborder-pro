...
export function useWorkflowState() {
  ...
  const nodeConfigsRef =
    useRef<Record<string, Record<string, unknown>>>(defaultConfigs);
  const [nodeConfigs, setNodeConfigsState] =
    useState<Record<string, Record<string, unknown>>>(defaultConfigs);
  const { settings: soundSettings } = useSoundSettings();

  // 根据 workflowNodeIds 从插件注册表查找节点定义
  const workflowNodes = state.workflowNodeIds // UI 渲染去使用
  ...
  if (!engineRef.current) {
    ...
    engine.registerMany(pluginRegistry.getExecutors()); // 引擎调用
  ...
  const addNode = useCallback((pluginId: string) => {
    setState((prev) => {
      ...
      const selIdx = prev.selectedNodeId
        ? prev.workflowNodeIds.indexOf(prev.selectedNodeId)
        : -1;
      const insertAt =
        selIdx >= 0
          ? selIdx + 1
  ...
  const moveNode = useCallback((nodeId: string, direction: "up" | "down") => {
    setState((prev) => {
      ...
      if (targetIdx < 0 || targetIdx >= prev.workflowNodeIds.length)
        return prev;
      // 不能越过 start 和 end
      if (
        prev.workflowNodeIds[targetIdx] === "start" ||
        prev.workflowNodeIds[targetIdx] === "end"
      )
        return prev;
      const newIds = [...prev.workflowNodeIds];
  ...
  const switchExecutionMode = useCallback(
    (mode: "auto" | "manual") => {
      ...
      addLog({
        ...
        nodeLabel: "系统",
        message:
          mode === "manual" ? "切换到手动单步模式" : "切换到自动连续模式",
        level: "info",
      });
    },
    [state.executionMode],
  ...
  const startExecution = useCallback(() => {
    ...
    setState((prev) => {
      ...
      return {
        ...
        nodeStatuses: Object.fromEntries(
          workflowNodes.map((n) => [n.id, "idle"] as const),
        ),
        stepOutputs: {},
      };
    });
    ...
    const activeTemplate = state.activeTemplateId
      ? getTemplateById(state.activeTemplateId)
      : null;
    ...
    const callbacks: EngineCallbacks = {
      onNodeStatus: (nodeId, status) => {
        ...
        if (status === "success" && soundSettings.nodeComplete)
          playNodeComplete();
      ...
      onComplete: () => {
        if (!cancelledRef.current) {
          ...
          persist(
            `/api/workflow/executions/${executionIdRef.current}/complete`,
            {
              status: "completed",
  ...
  const testNode = useCallback(
    (nodeId: string) => {
      ...
      addLog({
        nodeId: "system",
        nodeLabel: "系统",
        message: `从「${stepNodes[startIndex].label}」开始测试`,
        level: "info",
      });
  ...
  const loadTemplate = useCallback((templateId: string) => {
    ...
    if (template.defaultConfigs) {
      for (const [nodeId, overrides] of Object.entries(
        template.defaultConfigs,
      )) {
